...
public class Count_of_Smaller_Numbers_After_Self {
	 ...
	 * 
	 * 將所有數字平移至全部大於 0 後 map 到 array, array 的 index 表示數字
	 * 這個 array 表示每個數字出現次數
	 * "修改" 表示將該數字所對應的元素 + 1, 即出現次數 + 1
	 * "求和" 表示將該數字前的所有元素全部加起來, 即統計比這個數字還小的元素的出現次數
	 * 
	 * Rf : 
	...
	private int get(int i, int[] tree) {      // 求和
		int num = 0;
		while (i > 0) {
			num += tree[i];
			i -= i & (-i);
	...
	private void update(int i, int[] tree) {  // 修改
		while (i < tree.length) {
			tree[i]++;
			i += i & (-i);
		}
